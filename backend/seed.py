    ...
    try:
        ...
        if os.path.exists(fac_path):
            try:
                ...
                logger.error(f"Failed to seed faculty: {e}")
                db.rollback()
        ...
        if os.path.exists(club_path):
            try:
                ...
                logger.error(f"Failed to seed clubs: {e}")
                db.rollback()
        ...
        if os.path.exists(notices_path):
            try:
                ...
                logger.error(f"Failed to seed notices: {e}")
                db.rollback()
        ...
        if os.path.exists(buildings_path):
            try:
                ...
                logger.error(f"Failed to seed buildings: {e}")
                db.rollback()

    except Exception as e:
        logger.error(f"Outer seeding loop failed: {e}")
        db.rollback()
    finally:
        db.close()
